...
function WorkoutPicker({ workouts, selectedId, onSelect }: { workouts: Awaited<ReturnType<typeof getStrengthWorkouts>>; selectedId: string; onSelect: (id: string) => void }) {
  const pending = workouts.filter((workout) => workout.status !== "completed");
  if (!pending.length) return null;
  return <section className="mt-6">
    <div className="mb-3 flex items-center gap-2"><CalendarDays size={18} className="text-emerald-400"/><h2 className="font-bold">Fichas da semana</h2></div>
    <div className="flex gap-3 overflow-x-auto pb-2">
      {pending.map((workout) => {
        const active = workout.id === selectedId;
        return <button key={workout.id} onClick={() => onSelect(workout.id)} className={`min-w-[230px] rounded-2xl border p-4 text-left transition ${active ? "border-emerald-500 bg-emerald-500/10" : "border-zinc-800 bg-zinc-900"}`}>
          <p className="text-xs uppercase tracking-wide text-zinc-500">{new Intl.DateTimeFormat("pt-BR", { weekday: "short", day: "2-digit", month: "2-digit" }).format(new Date(`${workout.date}T12:00:00`))}</p>
          <p className="mt-2 line-clamp-2 font-bold text-white">{workout.title}</p>
          <p className="mt-1 line-clamp-2 text-xs text-zinc-400">{workout.focus || "Força complementar ao ciclismo"}</p>
        </button>;
      })}
    </div>
  </section>;
...
function WorkoutSheet({ workout }: { workout: Awaited<ReturnType<typeof getStrengthWorkouts>>[number] }) {
  ...
  return <div className="mt-8 space-y-5">
    ...
    {workout.exercises.map(exercise => <section key={exercise.id} className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900">
      ...
      <div className="space-y-3 p-4 md:hidden">{exercise.sets.map((set) => {
        const d = drafts[set.id] ?? {};
        return <div key={set.id} className={`rounded-2xl border p-4 ${set.completed ? "border-emerald-500/30 bg-emerald-500/5" : "border-zinc-800 bg-zinc-950/60"}`}>
          <div className="flex items-center justify-between"><p className="font-bold">Série {set.setNumber}</p>{set.completed ? <span className="inline-flex items-center gap-1 text-xs text-emerald-400"><CheckCircle2 size={15}/> Concluída</span> : <span className="text-xs text-zinc-600">Pendente</span>}</div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <MobileInput label="Carga (kg)" value={d.performedLoadKg ?? set.performedLoadKg ?? set.plannedLoadKg ?? ""} onChange={(v) => setDrafts((x) => ({...x,[set.id]:{...x[set.id],performedLoadKg:v}}))}/>
            <MobileInput label="Repetições" step="1" value={d.performedReps ?? set.performedReps ?? set.plannedReps ?? ""} onChange={(v) => setDrafts((x) => ({...x,[set.id]:{...x[set.id],performedReps:v}}))}/>
            <MobileInput label="RPE" value={d.rpe ?? set.rpe ?? ""} onChange={(v) => setDrafts((x) => ({...x,[set.id]:{...x[set.id],rpe:v}}))}/>
            <MobileInput label="RIR" value={d.rir ?? set.rir ?? ""} onChange={(v) => setDrafts((x) => ({...x,[set.id]:{...x[set.id],rir:v}}))}/>
          </div>
          <button onClick={() => saveSet(set)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 font-bold text-zinc-950"><Save size={17}/> Salvar série</button>
        </div>;
      })}</div>
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[720px] text-sm"><thead className="bg-zinc-950/60 text-zinc-500"><tr><th className="p-3 text-left">Série</th><th className="p-3">Carga kg</th><th className="p-3">Reps</th><th className="p-3">RPE</th><th className="p-3">RIR</th><th className="p-3">Status</th><th className="p-3"></th></tr></thead><tbody>{exercise.sets.map(set => {
...
function MobileInput({ label, value, onChange, step = "0.5" }: { label: string; value: number | string; onChange: (v: number | null) => void; step?: string }) { return <label><span className="mb-1 block text-[11px] uppercase tracking-wide text-zinc-500">{label}</span><input type="number" step={step} value={value} onChange={(event) => onChange(event.target.value === "" ? null : Number(event.target.value))} className="w-full rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-3 text-base outline-none focus:border-emerald-500"/></label>; }
...
function HistoryView({ workouts, progress }: { workouts: Awaited<ReturnType<typeof getStrengthWorkouts>>; progress: Awaited<ReturnType<typeof getStrengthProgress>> }) {
  const completedWorkouts = workouts.filter((workout) => workout.status === "completed").slice(0, 12);
  return <div className="mt-8 space-y-8">
    <section><h2 className="text-xl font-bold">Treinos concluídos</h2><p className="mt-1 text-sm text-zinc-500">Suas fichas anteriores ficam registradas para orientar a progressão da IA.</p>
      {completedWorkouts.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{completedWorkouts.map((workout) => <article key={workout.id} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4"><p className="text-xs uppercase text-zinc-500">{new Intl.DateTimeFormat("pt-BR").format(new Date(`${workout.date}T12:00:00`))}</p><h3 className="mt-2 font-bold">{workout.title}</h3><p className="mt-1 text-xs text-zinc-400">{workout.focus || "Treino de força"}</p><p className="mt-3 text-xs text-emerald-400">{workout.exercises.length} exercícios · concluído</p></article>)}</div> : <div className="mt-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-6 text-sm text-zinc-500">Nenhuma ficha concluída ainda.</div>}
    </section>
    <section><h2 className="text-xl font-bold">Progressão de carga</h2><ProgressView progress={progress}/></section>
  </div>;
}
